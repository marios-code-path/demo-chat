...
interface ChatMessageByUserRepository<T : Any> : ReactiveCassandraRepository<ChatMessageByUser<T>, T> {
    fun findByKeyFrom(userId: T): Flux<ChatMessageByUser<T>>
    fun deleteByKeyId(msgId: T): Mono<Void>
}

interface ChatMessageByTopicRepository<T : Any> : ReactiveCassandraRepository<ChatMessageByTopic<T>, T> {
