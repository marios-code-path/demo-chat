...
**The `--ci` mode was measured on 2026-09-22** at master `67762d51`, against Docker Engine 29.7.2. It exits 0 and reports no drift. 27 modules run 1124 tests, with 0 failures, 0 errors and 54 skipped. The same mode reported 1080 at master `c6c21f12` and 1082 at master `67762d51`. The count was 1075 on 2026-09-20. `CHAT-hazcatpc` added three tests, and the default count records those three. `CHAT-sgyaaivp` added two tests that carry the `integration` tag, so only `--ci` runs them. `CHAT-cophllrg` added two tests. `CHAT-ltvfmcvh` replaced its eight diagnosis tests with seven policy tests, `CHAT-gtebuipo` added one, `CHAT-qucgqaye` added six, and `CHAT-vehpbvzn` added seven, and the grant order clock added twenty one. `--ci` resolves artifacts online, so the measured command is what `just check-ci` runs.
...
skipped. The reactor holds 36 modules and reports 891 tests, 0 failures, 0
...
891 when the grant order clock landed.
