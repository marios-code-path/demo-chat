...
`CHAT-czmjffen` adds six integration tests, three per Redis backend. Both gates
...
reports 1065 tests, 0 failures, 0 errors and 55 skipped, so it runs 1010. Both
exit 0 and report no failure-list drift. These are measured counts, not counts
derived from an earlier run.

The default count does not move, because the six new tests sit in classes
tagged `integration`.
